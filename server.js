    ...
    ws.on('message', (data, isBinary) => {
      try {
        ...
        switch (msg.type) {
          ...
          case 'webrtc_answer':
          case 'webrtc_ice': {
            const targetDashboard = msg.targetDashboard;
            if (!targetDashboard) break;
            
            for (const d of dashboards) {
              if (d.sessionId === targetDashboard && d.ws.readyState === WebSocket.OPEN) {
                msg.agentId = agentId;
    ...
    ws.on('message', (data) => {
      try {
        ...
        switch (msg.type) {
          ...
          case 'webrtc_offer':
          case 'webrtc_ice': {
            const targetId = msg.targetAgent;
            if (!targetId) break;
            
            msg.dashboardId = sessionId;
            sendToAgent(targetId, msg);
            break;
          }
